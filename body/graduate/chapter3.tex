...
基于上述分析，本章将数据关联问题建模为一般图匹配（General Graph Matching）问题。如\autoref{fig:ch3_1}所示，本章摒弃了跨帧连接的二分图方式（如\autoref{fig:ch3_1}(a)），转而
...
\section{双图协同关联跟踪框架}
\label{sec:ch3_2}
在上一小节中，本章将在线多目标跟踪的数据关联问题形式化为一般图匹配问题，并指出通过帧内构图与节点嵌入学习，可以在降低计算复杂度的同时引入更丰富的上下文信息。本节在此基础上，详细介绍所提出的双图协同关联跟踪框架（DGCTracker）的整体设计与实现流程。
\begin{figure}[htbp]
    \centering
...
为了避免全连接带来的计算冗余，本章采用 K-近邻（KNN）算法基于空间欧氏距离自适应生成稀疏拓扑结构；同时，融合目标的外观纹理与几何坐标信息，对图中的节点与边特征进行初始化编码，为后续的消息传递奠定基础。

\textbf{2. 双图特征提取与融合}：这是整个框架的核心组成部分。对构建的轨迹图与检测图，并行执行静态图卷积（Static GCN）和动态图卷积（Dynamic GCN）进行特征提取。Static GCN在固定的空间邻域中聚合信息，捕捉目标间稳定的局部几何结构；
...
为了在降低计算复杂度的同时保留关键的局部空间上下文，本章舍弃了全连接图结构，转而采用基于空间距离的稀疏构图策略。针对历史轨迹集合$\mathcal{L}^{t-1}$与检测集合$\mathcal{D}^{t}$，分别构建轨迹图$G^l=(V^l,E^l)$与检测图$G^d=(V^d,E^d)$。
...
在边集合$E^l$的构建上，本章遵循“空间局部性”的假设：即相互靠近的目标之间更可能存在复杂的交互关系（如群体运动、遮挡、碰撞），而距离较远的目标间的影响则相对微弱。若构建全连接图，不仅引入了大量无用的噪声连接，还会导致计算量随节点数量呈平方级增长。

因此，本章采用K-近邻（K-Nearest Neighbors，KNN）算法来动态生成稀疏连接。具体而言，对于图中的任意节点$v_i$，计算其与其他所有节点在图像平面上的欧式距离，并选取距离最近的$k$个邻居节点$\mathcal{N}_k(v_i)$建立有向边：
\begin{equation}
E^l = \{ e_{ji} \mid v_j \in \mathcal{N}_k(v_i),\ \forall\, v_i \in V^l \}
\end{equation}

通过这种策略，图中的每个节点仅与其局部邻域内的$k$个目标（默认设置为$k=2$）进行交互，将图的规模限制在线性复杂度$\mathcal{O}(kN)$，从而有效地降低了高密度场景下的计算瓶颈，为后续的图卷积操作提供高效的结构基础。
...
\textbf{边嵌入初始化}：为了充分利用目标间的空间结构关系，本章将节点间的相对几何差异编码为边特征，作为后续消息传递过程中重要的关系先验。对于一条从源节点$v_j$指向目标节点$v_i$的有向边$e_{ji}$，计算一个包含5个分量的归一化几何向量：
\begin{equation}
...
在此统一建模框架下，本章进一步从关系建模的角度出发，将图$G$拆解为静态图$^{s}G$（Static Graph）与动态图$^{d}G$（Dynamic Graph）两种互补表示，分别用于刻画稳定的空间结构关系与判别性的语义差异关系。为了消除概念上的混淆，我们将两者的本质区别定义如下：
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{几何运动一致性（Geometric Consistency）}：为了缓解行人姿态变化导致的包围框宽度不稳定性，本章采用高度感知交并比（HIoU） 。HIoU 仅基于高度方向计算重叠率，从而将几何约束重点聚焦于目标的深度一致性，有效剔除物理空间上不合理的候选匹配，提升了关联层对遮挡和形变的鲁棒性。
\end{itemize}
...
为此，本章采用 Focal Loss \cite{focalloss}对亲和矩阵进行监督学习，
以增强模型对难分类样本的关注。
...
在推理阶段，为获得满足严格互斥约束的一对一离散匹配结果，本章将$\hat{S}$转换为代价矩阵并采用匈牙利算法求解最优二分匹配。具体地，可定义代价为：
\begin{equation}
    C=1-\hat{S}
\end{equation}
...
本阶段旨在实现高可见度、高置信度目标的精确关联。将活跃轨迹集$L_{\text{active}}$与高置信度检测框$D_{\text{high}}$分别构建为轨迹图与检测图，并利用本章核心方法——双图协同关联网络（DGC）计算二者间的亲和度矩阵$\hat{S}$：
\begin{equation}
...
最终，系统输出更新后的轨迹集合$\mathcal{L}^t$，完成当前帧的跟踪。该三阶段级联策略通过精细化、分优先级的资源分配，在确保高精度关联的基础上，显著提升了系统对复杂场景的适应性与轨迹的长期一致性。
...
其次，\ref{subsec:ch3_4_2} 节将本方法与当前主流的先进算法进行对比，从客观指标和主观可视化两个维度评估算法的综合性能；
...
此外，为了公平地评估本章提出的关联算法性能，并排除检测器质量差异带来的干扰，本节所有实验均严格遵循 Public Detection（公共检测）协议，即直接使用数据集官方提供的检测结果，而不引入额外的私有检测器进行推理。具体而言，在 MOT16 上使用 DPM 检测器结果，在 MOT17 上使用 SDP 检测器结果。
...
DGCTracker 的实现基于 PyTorch 与 PyTorch-Geometric 框架完成，并在一张 NVIDIA RTX 4090 GPU 上进行训练。
...
为了验证本章提出的 DGCTracker 算法在复杂场景下的综合性能，本节在 MOT16 和 MOT17 测试集上进行了定量对比与定性分析。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccccccc}
    ...
    \textbf{有向图 (Default)} & \textbf{61.65} & 56.42 & \textbf{67.56} & \textbf{72.64} & \textbf{61.25} & \textbf{89.25} & \textbf{65.59} & \textbf{84.67} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccccccc}
    ...
    \textbf{有向无环图(Default)} & \textbf{61.65} & \textbf{56.42} & \textbf{67.56} & \textbf{72.64} & \textbf{61.25} & \textbf{89.25} & 65.59 & 84.67 \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
为保证算法在该类场景下的健壮性，本章在工程实现中采用如下策略：当图中节点数为 1 时，默认补充自环边以保证图结构有效；当节点数大于 1 时，则使用有向无环图作为默认配置。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccccccc}
    ...
    \textbf{基于余弦距离(Default)} & \textbf{61.65} & \textbf{56.42} & \textbf{67.56} & \textbf{72.64} & \textbf{61.25} & \textbf{89.25} & \textbf{65.59} & 84.67 \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
\begin{figure}[htbp]
    ...
    \caption{\label{fig:ch_3_16}14个节点的图结构随$K$值增加而演变的示意图}
\end{figure}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccccccc}
    ...
    \midrule
    \textbf{K=2 (Default)} & \textbf{61.65} & \textbf{56.42} & \textbf{67.56} & 72.64 & 61.25 & 89.25 & 65.59 & 84.67 \\
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccccccc}
    ...
    \textbf{有残差连接 (Default)} & \textbf{61.65} & \textbf{56.42} & \textbf{67.56} & \textbf{72.64} & \textbf{61.25} & \textbf{89.25} & \textbf{65.59} & \textbf{84.67} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
\begin{equation}
\begin{dcases}
    ^{s}h_i^{\beta} = {^{s}g^\beta}\left(  ^{s}f_1^\beta\left( ^{s}h_i^{\beta-1} \right) + \frac{1}{N}\sum_{j \in \mathcal{N}(i)} {^{s}f_2}^\beta\left( ^{s}z_{ji} ~, \left( ^{s}h_j^{\beta-1} - {^{s}h_i^{\beta-1}} \right) \right) \right) \\
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccccccc}
    ...
    \textbf{Max Pooling (Default)} & \textbf{61.65} & \textbf{56.42} & \textbf{67.56} & \textbf{72.64} & \textbf{61.25} & \textbf{89.25} & \textbf{65.59} & \textbf{84.67} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccccccc}
    ...
    \textbf{GraphNorm (Default)} & \textbf{61.65} & \textbf{56.42} & \textbf{67.56} & \textbf{72.64} & \textbf{61.25} & \textbf{89.25} & \textbf{65.59} & \textbf{84.67} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccccccc}
    ...
    \textbf{3s + 2d (Default)} & \textbf{61.65} & \textbf{56.42} & \textbf{67.56} & \textbf{72.64} & \textbf{61.25} & \textbf{89.25} & \textbf{65.59} & \textbf{84.67} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
从而学习到更具判别力的表示。这验证了本章设计的层次化特征提取架构的有效性。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccccccc}
    ...
    \textbf{Triple (Default)} & \textbf{61.65} & \textbf{56.42} & \textbf{67.56} & \textbf{72.64} & \textbf{61.25} & \textbf{89.25} & \textbf{65.59} & \textbf{84.67} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
最终，三种信息联合使用时达到最优结果，验证了本章在亲和建模阶段引入多模态融合策略的合理性与必要性。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccccccc}
    ...
    \textbf{w/ Augmentation (Default)} & \textbf{61.65} & \textbf{56.42} & \textbf{67.56} & \textbf{72.64} & \textbf{61.25} & \textbf{89.25} & \textbf{65.59} & \textbf{84.67} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
不同于传统跨帧稠密构图并进行边分类的范式，本章将数据关联重新建模为帧内图表征学习与跨图匹配问题：分别对历史轨迹集合与当前检测集合进行稀疏构图，通过图神经网络学习包含上下文信息的节点嵌入，再基于节点嵌入相似性完成跨帧匹配，从而在降低计算冗余的同时增强关联判别能力。
...
在匹配阶段，本章进一步融合图上下文相似度、外观相似度与几何一致性（HIoU）构建多模态亲和，并引入 Sinkhorn 可微归一化实现端到端的软指派优化，推理阶段结合匈牙利算法得到满足互斥约束的离散匹配结果。
